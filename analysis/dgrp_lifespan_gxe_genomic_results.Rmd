...
In general, prediction accuracy is lower in this scenario. The results show that, as expected, E-BLUP predicts poorly. G-BLUP predicts decently
because all the lines are present in the test set. GE-BLUP improves accuracy over G-BLUP. GxE-BLUP is worse than G-BLUP and GE-BLUP, on average.
However, when looking at the temperature-specific accuracies, we can see that it is only the prediction of lifespan at 28C that has lower accuracy.
