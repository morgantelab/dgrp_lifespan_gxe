...
Prediction accuracy was evaluated as the correlation coefficient between observed and predicted phenotypes in the test set. The two sexes were analyzed separately.
...
```{r random within env females, fig.height=10, fig.width=13}
sex <- 1
temp <- c(18,25,28)
model <- "T"
...
colnames(res) <- c("rep", "temp", "model", "r")

  for(t in temp){
    for(repp in 1:n_reps){
      dat <- readRDS(paste0("output/T_fit/", prefix, "_random_within_env_", sex, "_", t, "_T_fit_", repp, ".rds"))
      i <- i + 1
...
res %>% group_by(temp) %>% summarise(mean_r=mean(r), se_r=(sd(r)/n_reps)) %>% as.data.frame()
```
...
```{r random lines females, fig.height=10, fig.width=13}
sex <- 1
...
for(sche in schemes){
  for(met in model){
    for(repp in 1:n_reps){
      dat <- readRDS(paste0("output/", met, "_fit/", prefix, "_", sche, "_", sex, "_", met, "_fit_", repp, ".rds"))
      i <- i + 1
...
p <- ggplot(res, aes(x = model, y = r, fill = model)) +
  ...
  labs(x = "Model", y = expression(italic(r)), title="") +
  theme_cowplot(font_size = 18) +
...
```{r random obs females, fig.height=10, fig.width=13}
sex <- 1
schemes <- "random_obs"
model <- c("T", "Et", "TandE", "TxE")
...
```{r random within env males, fig.height=10, fig.width=13}
sex <- 0
temp <- c(18,25,28)
model <- "T"
...
The results show that prediction accuracy is pretty similar across environments.
...
```{r random lines males, fig.height=10, fig.width=13}
sex <- 0
schemes <- "random_lines"
model <- c("T", "Et", "TandE", "TxE")
...
effects are equal across environments, which is not the case. E-BLUP does very well in this scenario because when combining 
the data across environments, a lot of the variance is explained by the environmental effects, which we have good power to 
estimate. TE-BLUP and TxE-BLUP give marginally higher accuracy than E-BLUP, although these accuracies are very similar.
...
```{r random obs males, fig.height=10, fig.width=13}
sex <- 0
schemes <- "random_obs"
model <- c("T", "Et", "TandE", "TxE")
...
The results show a similar pattern to the previous scenario. However, the difference between (TE-BLUP, TxE-BLUP) and E-BLUP is larger than 
in the previous scenario, due to the full training-test transfer of information.
